...
void compose_imgui_frame()
{
  ...
  // control window
  {
    ...
    ImGui::SliderFloat3("Tranlsate", glm::value_ptr(g_vec_model_translate), -10.0f, 10.0f);
    ImGui::gizmo3D("Rotation", g_quat_model_rotation);
    ImGui::SliderFloat3("Scale", glm::value_ptr(g_vec_model_scale), 0.1f, 2.0f);
  ...
  // control window
  {
    ...
    bool is_perspective = g_camera.mode() == Camera::kPerspective ? true : false;
  ...
  // output window
  {
    ...
    glm::mat4 proj_mat = g_camera.get_projection_matrix();
    ImGui::TextColored(ImVec4(1,1,0,1), "projection matrix");
    ImGui::Text("%.3f %.3f %.3f %.3f ", proj_mat[0][0], proj_mat[1][0], proj_mat[2][0], proj_mat[3][0]);
    ImGui::Text("%.3f %.3f %.3f %.3f ", proj_mat[0][1], proj_mat[1][1], proj_mat[2][1], proj_mat[3][1]);
    ImGui::Text("%.3f %.3f %.3f %.3f ", proj_mat[0][2], proj_mat[1][2], proj_mat[2][2], proj_mat[3][2]);
    ImGui::Text("%.3f %.3f %.3f %.3f ", proj_mat[0][3], proj_mat[1][3], proj_mat[2][3], proj_mat[3][3]);
    ImGui::NewLine();
...
{
  // view matrix
  g_mat_view = g_camera.get_view_matrix();

  // projection matrix
  g_mat_proj = g_camera.get_projection_matrix();

  // model matrix = T*R*S
  g_mat_model = glm::mat4(1.0f);
  g_mat_model = g_mat_model * glm::translate(g_vec_model_translate);
  g_mat_model = g_mat_model * glm::mat4_cast(g_quat_model_rotation);
  g_mat_model = g_mat_model * glm::scale(glm::mat4(1.0f), g_vec_model_scale);
}
